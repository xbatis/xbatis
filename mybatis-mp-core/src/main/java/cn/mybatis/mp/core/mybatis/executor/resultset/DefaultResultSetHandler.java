 ...
 */
public class DefaultResultSetHandler extends org.apache.ibatis.executor.resultset.DefaultResultSetHandler {
    ...
    private List<UnMappedColumnAutoMapping> createAutomaticMappings(ResultSetWrapper rsw, ResultMap resultMap,
        ...
        if (autoMapping == null) {
             ...
             */
            //是否是Map映射
            boolean isForMap = Map.class.isAssignableFrom(resultMap.getType()) && resultMap.getResultMappings().isEmpty() && resultMap.getIdResultMappings().isEmpty();
            //是否全大写
            boolean isAllColumnUpperCase = true;
            for (String columnName : unmappedColumnNames) {
                if (!isAllUpperCase(columnName)) {
                    isAllColumnUpperCase = false;
                    break;
                }
            }
            ...
            for (String columnName : unmappedColumnNames) {
                ...
                if (isForMap) {
                    ...
                    property = metaObject.findProperty(propertyName, false);
                }
    ...
    private boolean applyArgNameBasedConstructorAutoMapping(ResultSetWrapper rsw, ResultMap resultMap,
        ...
        if (foundValues && constructorArgs.size() < params.length) {
            throw new ExecutorException(MessageFormat.format(
                    ...
                    missingArgs, constructor, rsw.getColumnNames(), false));
        }
        return foundValues;
    }
    ...
    private boolean columnMatchesParam(String columnName, String paramName, String columnPrefix) {
        ...
        return paramName
                .equalsIgnoreCase(false ? columnName.replace("_", "") : columnName);
    }
    ...
    private void createRowKeyForUnmappedProperties(ResultMap resultMap, ResultSetWrapper rsw, CacheKey cacheKey,
        ...
        for (String column : unmappedColumnNames) {
            ...
            if (metaType.findProperty(property, false) != null) {
