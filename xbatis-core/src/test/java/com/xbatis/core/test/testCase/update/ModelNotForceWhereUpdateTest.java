...
public class ModelNotForceWhereUpdateTest extends BaseTest {
    ...
    public void notForceInsertTest3() {
        try (SqlSession session = this.sqlSessionFactory.openSession(false)) {
            SysUserMapper sysUserMapper = session.getMapper(SysUserMapper.class);
            SysUserModel sysUserModel = QueryChain.of(sysUserMapper)
                    .returnType(SysUserModel.class)
                    .eq(SysUser::getId, 1)
                    .get();
            sysUserModel.setId(null);
            sysUserModel.setUserName(null);

            UpdateStrategy<SysUserModel> updateStrategy = UpdateStrategy.create();
            updateStrategy.on(WhereUtil.create().eq(SysUser::getId, 1));
            updateStrategy.forceFields(SysUserModel::getRole_id);
