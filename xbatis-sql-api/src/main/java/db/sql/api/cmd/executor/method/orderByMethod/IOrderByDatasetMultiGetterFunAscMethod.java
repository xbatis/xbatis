...
public interface IOrderByDatasetMultiGetterFunAscMethod<SELF extends IOrderByDatasetMultiGetterFunAscMethod> extends IOrderByDatasetMultiGetterFunMethod<SELF> {
    ...
    default <DATASET extends IDataset<DATASET, DATASET_FIELD>, DATASET_FIELD extends IDatasetField<DATASET_FIELD>> SELF orderByAsc(boolean when, IDataset<DATASET, DATASET_FIELD> dataset, GetterField[] getterFields, Function<IDatasetField[], Cmd> f) {
        if (!when) {
            return (SELF) this;
        }
        return this.orderBy(dataset, ascOrderByDirection(), getterFields, f);
